...
{    $srStatus = $HTTP_GET_VARS['status'];
  ...
  if (!is_numeric($srStatus)) {
   ...
   // order vendor
  if ( ($HTTP_GET_VARS['vendor']) && (tep_not_null($HTTP_GET_VARS['vendor'])) ) 
{    $srVendor = $HTTP_GET_VARS['vendor'];
  }
...
{    $sYear = $HTTP_GET_VARS['startY'];
  ...
  $sr = new sales_report($srView, $startDate, $endDate, $srSort, $srStatus, $srVendor, 
